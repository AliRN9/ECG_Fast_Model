...
mesh = meshio.read("mesh/real_model/mesh_biv_fixed.vtu")
# print(dir(mesh))
...
def temp(x, A_residual, el):
    vm = np.loadtxt(el)
    return dot(x, A_residual * vm)

# @nb.jit(parallel=True)
def ECG(x:np.array):
    # print(A_residual)

    data = glob.glob('mesh/real_model/vm_biv_fixed_600/*.txt')
    data = sorted(data)
    A_residual = asm(laplace_A_residual, basis, sigma_i=basis3x3.interpolate(sigma_i.flatten()))

    with Pool(4) as pool:
        values = pool.starmap(temp, [(x,A_residual, el) for el in data])

        # print(i)

        # values.append(ECG_value)
    # values = []
    # data = glob.glob('vm_biv_fixed_600/*.txt')
    # data = sorted(data)
    #
    # for el in data:
    #     # print(i)
...
def ECG_plot(delta):
    ...
    plt.plot(time, delta)
    plt.title('FL метод на реальной моделb')
